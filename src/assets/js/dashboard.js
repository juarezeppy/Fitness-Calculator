...
var handleDashboardVectorMap = function() {
	"use strict";
	if ($('#dashboard-world-map').length !== 0) {
		$('#dashboard-world-map').vectorMap({
		map: 'world_mill_en',
...
var handleDashboardInteractiveChart = function () {
	"use strict";
    function showTooltip(x, y, contents) {
        $('<div id="flot-tooltip" class="flot-tooltip">' + contents + '</div>').css( {
            top: y - 45,
            left: x - 55
        }).appendTo("body").fadeIn(200);
    }
	if ($('#dashboard-interactive-chart').length !== 0) {
        ...
        $.plot($("#dashboard-interactive-chart"), [
                {
                    data: data1, 
                    label: "Page Views", 
                    color: blue,
        ...
        $("#dashboard-interactive-chart").bind("plothover", function (event, pos, item) {
...
var handleDashboardDonutChart = function () {
	"use strict";
	if ($('#dashboard-donut-chart').length !== 0) {
		...
		$.plot('#dashboard-donut-chart', donutData, {
			series: {
				pie: {
					innerRadius: 0.5,
					show: true,
...
var Dashboard = function () {
    ...
    return {
        ...
        init: function () {
                ...
                $.getScript('assets/plugins/flot/jquery.flot.time.min.js').done(function() {
                    $.getScript('assets/plugins/flot/jquery.flot.resize.min.js').done(function() {
                        $.getScript('assets/plugins/flot/jquery.flot.pie.min.js').done(function() {
                            handleDashboardInteractiveChart();
                            handleDashboardDonutChart();
                        });
                    });
                });
            });
            ...
            $.getScript('assets/plugins/jquery-jvectormap/jquery-jvectormap.min.js').done(function() {
